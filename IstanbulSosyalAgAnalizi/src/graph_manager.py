...
class Graph:
    ...
    def add_node(self, node):
        if node.id not in self.nodes:
            ...
            print(f"[UYARI] Düğüm ID {node.id} zaten var. İşlem iptal edildi.")
            return False

    def add_edge(self, id1, id2):
        """İki düğüm arasında dinamik ağırlıklı ve yönsüz bir kenar oluştur."""
        try:
    ...
    def save_to_json_path(self, file_path):
        """Graf verilerini kullanıcı tarafından seçilen konuma JSON olarak kaydet."""
        try:
            data = {"nodes": [], "edges": []}
    ...
    def load_from_json(self, file_path=None):
        """Dışarıdan gelen dosya yoluna göre grafı yükle."""
        # Eğer özel bir yol verilmemişse varsayılan dosyaya bak
        path = file_path if file_path else os.path.join(self.data_dir, "kaydedilen_veriler.json")
        
        try:
            with open(path, "r", encoding="utf-8") as f:
                data = json.load(f)
            
            # Mevcut grafı temizle
            self.nodes = {}
            self.adjacency_list = {}
            ...
            for n_data in data["nodes"]:
                node = Node(
                    ...
                    n_data['active_score'], n_data['social_score'], n_data['connection_count']
                )
                self.add_node(node)
            ...
            print(f"[BAŞARILI] JSON yüklendi: {path}")
            return True
        except Exception as e:
            print(f"[HATA] JSON Yükleme: {e}")
            return False
    ...
    def get_neighbors(self, node_id):
        """Belirli bir düğümün komşuluk listesini döndürür."""
    ...
    def get_adjacency_matrix_data(self):
        """Tabloda gösterilmek üzere matris verisini ve başlıkları hazırlar."""
        sorted_ids = sorted(self.nodes.keys())
        size = len(sorted_ids)
        matrix = [[0] * size for _ in range(size)]
        id_map = {node_id: idx for idx, node_id in enumerate(sorted_ids)}
